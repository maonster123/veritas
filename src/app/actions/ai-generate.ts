...
export async function generateAIContent(nodeId: string): Promise<{ success: boolean; content?: string; error?: string }> {
  try {
    ...
    const rl = await checkRateLimit(session.user.id, "ai-generate", 20, 60);
    if (!rl.allowed) return { success: false, error: `请求过于频繁，请 ${Math.ceil((rl.resetAt.getTime() - Date.now()) / 60000)} 分钟后再试` };

    // Subscription limit
    const aiCheck = await checkAILimit(session.user.id);
    if (!aiCheck.allowed) return { success: false, error: "AI 次数已用完，请升级套餐" };
...
7. 句式多样：长短句交替。`;
    ...
    await recordRateLimit(session.user.id, "ai-generate");
...
export async function normalizeCitation(
  ...
  try {
    ...
    // Rate limit for AI citation normalization: 15 per hour
    const rlCite = await checkRateLimit(session.user.id, "cite-norm", 15, 60);
    if (!rlCite.allowed) return { success: false, error: `请求过于频繁，请稍后再试` };

    const aiCite = await checkAILimit(session.user.id);
    if (!aiCite.allowed) return { success: false, error: "AI 次数已用完，请升级套餐" };
    ...
    await recordRateLimit(session.user.id, "cite-norm");
    await incrementAIUsage(session.user.id);
...
export async function generateKeywords(
  ...
  try {
    ...
    const rlKw = await checkRateLimit(session.user.id, "keywords", 10, 60);
    if (!rlKw.allowed) return { success: false, error: "请求过于频繁，请稍后再试" };

    const aiKw = await checkAILimit(session.user.id);
    if (!aiKw.allowed) return { success: false, error: "AI 次数已用完，请升级套餐" };
...
6. 只返回逗号分隔的关键词。不要编号，不要解释。`;
    ...
    await recordRateLimit(session.user.id, "keywords");
    await incrementAIUsage(session.user.id);
